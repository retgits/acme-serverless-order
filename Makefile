...
build: ## Build the executable for Lambda
	echo
	GOOS=linux GOARCH=amd64 go build -o bin/addorder $(if $V,-v) $(version_flags) ./addorder
	GOOS=linux GOARCH=amd64 go build -o bin/getallorders $(if $V,-v) $(version_flags) ./getallorders
	GOOS=linux GOARCH=amd64 go build -o bin/getordersbyuser $(if $V,-v) $(version_flags) ./getordersbyuser
	GOOS=linux GOARCH=amd64 go build -o bin/sendtoshipment $(if $V,-v) $(version_flags) ./sendtoshipment
	GOOS=linux GOARCH=amd64 go build -o bin/updatedeliverystatus $(if $V,-v) $(version_flags) ./updatedeliverystatus
	echo
